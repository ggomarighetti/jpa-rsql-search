            ...
            try (InputStream input = zip.getInputStream(entry)) {
                ...
                assertThat(properties)
                        .allSatisfy(property -> assertThat(property.path("description").asText())
                                ...
                                .isNotBlank());
                assertThat(properties)
                        .allSatisfy(property -> assertThat(property.path("name").asText())
                                .startsWith("rsql.jpa.search."));
                assertThat(properties)
                        .noneSatisfy(property -> assertThat(property.path("name").asText())
                                .startsWith("jpa.rsql.search."));

                assertProperty(properties, "rsql.jpa.search.rsql.enabled", "java.lang.Boolean", "true");
                assertProperty(
                        properties,
                        "rsql.jpa.search.filter.max-comparisons",
                        "java.lang.Integer",
                        null);
                assertProperty(
                        properties,
                        "rsql.jpa.search.rsql.perplexhub.strict-equality",
                        "java.lang.Boolean",
                        "true");
            }
        }
